...
generate_ciphertext_alphabet: # int generate_ciphertext_alphabet (string ciphertext_alphabet, string keyphrase)
	...
	# a1 = keyphrase
	# v0 -> # of unique case-sensitive alphanumerical characters in keyphrase
	li $v0, 0 # start at 0
	...
	gen_ciphertext_alphabet_from_keyphrase:
		lbu $t1, 0($t0) # letter from keyphrase
		addi $t0, $t0, 1 # increment keyphrase
		beq $t1, $0, add_the_rest_of_letters # t1 == \0 -> keep checking keyphrase
		# check char is added
		# t3 = use to get numerical counting value of t1
		# t4 = offset to get index in letter set	
		# t5 = the upper bound for letter 	
		li $t3, '0'
		li $t4, 52  # position offset in letter set
		blt $t1, $t3, gen_ciphertext_alphabet_from_keyphrase # t1 < '0' not alphanumerical -> next char
		li $t5, '9' 
		ble $t1, $t5, check_char_to_add # '0' <= t0 <= '9' is digit 
		
		li $t3, 'A' 
		addi $t4, $t4, -26 # decrement position offset in letter set
		blt $t1, $t3, gen_ciphertext_alphabet_from_keyphrase # '9' < t0 < 'A' not alphanumerical -> next char
		li $t5, 'Z'
		ble $t1, $t5, check_char_to_add # 'A' <= t0 <= 'Z' is uppercase
		
		li $t3, 'a' 
		addi $t4, $t4, -26 # decrement position offset in letter set
		blt $t1, $t3, gen_ciphertext_alphabet_from_keyphrase # 'Z' < t0 < 'a' not alphanumerical -> next char
		li $t5, 'z'
		ble $t1, $t5, check_char_to_add # 'a' <= t0 <= 'z' is lowercase
		
		j gen_ciphertext_alphabet_from_keyphrase
		
		check_char_to_add:
			sub $t3, $t1, $t3 # numerical counting value of character set
			add $t3, $t3, $t4 # t3 = t3 + t4 (position for character set in list)
			add $t2, $sp, $t3 # the index of the list of characters to change to 0
			lbu $t5, 0($t2) # check if value is 0 or not. if 0 don't add
			beqz $t5, gen_ciphertext_alphabet_from_keyphrase
		add_char_to_ciphertext:
			sb $0, 0($t2) # 0 out the character to be added
	...
	add_the_rest_of_letters:
		...
		li $t0, 0 # counter
		li $t7, 63 # use to end add_the_rest_of_lettes_loop
